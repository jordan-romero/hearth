...
*entire* production schema so we know Phase 0's lean core is a clean **subset**,
not a dead-end. We still only *build* each group in its phase — but nothing here
...
| Entity | Purpose | Phase |
|---|---|---|
| `KnowledgeUnit` | The atom: content, `type`, `source`, `origin` ⚑, `baseVisibility`, `campaignId`, `gameSessionId?` ⚑, `sourceDocumentId?`, `subjectId?/objectId?` ⚑ (relationships) | 0 |
...
| *(live rolling transcript)* | The bathroom-break buffer — likely **in-memory/Redis, not a table** | 4 |

> **Transcripts are _source_, not memory.** Raw segments are the *input* to
> extraction; `SESSION` KnowledgeUnits point back to them for provenance. The
> permission filter runs over *extracted units*, never over raw transcript — so
...
| *(NPC generator)* | Writes `KnowledgeUnit` with `origin = GENERATED`, linked to its `GenerationRun` — **no new table** | 3 |
...
> regenerated when the underlying memory changes. This is *why* we log generations.

## F · Agent runs / eval / ops
| Entity | Purpose | Phase |
|---|---|---|
| `GenerationRun` | Every Claude call: `kind`, `promptVersion`, `model`, tokens, latency, cost, status | 3 |
| *(prompts)* | **Versioned files in the repo**, not a table — `promptVersion` on the run points at them | 3 |
...
| *(theme)* | `Campaign.theme` preset enum (+ optional overrides later) | 5 |

## I · Billing
| Entity | Purpose | Phase |
|---|---|---|
| `Subscription` / `Customer` | Campaign/User ↔ Stripe ids, plan, status | 6 |
| *(usage)* | Metered off `GenerationRun` (no separate ledger to start) | 6 |

---

## Why the lean start is safe (the load-bearing principles)
1. **Additive migrations.** Every group B→I is *new* tables/columns added in its
